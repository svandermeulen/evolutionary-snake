...
def test_boundary_factory(game_settings: settings.GameSettings) -> None:
    """Test the boundary factory."""
    boundary = boundary_factory.boundary_factory(settings=game_settings)
    assert isinstance(boundary, boundaries.PeriodicBoundary)


def test_boundary_factory_with_invalid_input(
    game_settings: settings.GameSettings,
) -> None:
    """Test the boundary factory with invalid settings."""
    game_settings.boundary_type = "invalid_boundary_type"  # type: ignore[assignment]
    with pytest.raises(NotImplementedError):
        boundary_factory.boundary_factory(settings=game_settings)
